    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('actions', function ($query) {
                $editBtn = "<a href='" . route('admin.brand.edit', $query->slug) . "' class='btn btn-primary btn-sm'><i class='far fa-edit'></i></a>";
                $deleteBtn = "<a href='" . route('admin.brand.destroy', $query->slug) . "' class='btn btn-danger ml-2 btn-sm delete-item'><i class='far fa-trash-alt'></i></a>";

                return $editBtn . $deleteBtn;
            })
            ->addColumn('logo', function ($query) {
                $imageUrl = $query->logo
                    ? asset('storage/logo/' . $query->logo)
                    : asset('default/image.jpg');

                return "<img width='100px' src='{$imageUrl}' alt='Logo' class='img-thumbnail'>";
            })
            ->addColumn('is_featured', function ($query) {
                $active = '<i class="badge badge-success">Si</i>';
                $inActive = '<i class="badge badge-danger">No</i>';
                if ($query->is_featured == 'yes') {
                    return $active;
                } else {
                    return $inActive;
                }
            ...
            ->rawColumns(['logo', 'is_featured', 'status', 'actions'])
            ->setRowId('id');
    }
